...
function AccountingTab() {
  const { bookings, customers, drivers, vehicles, invoices, expenses, income, addExpense, updateExpense, deleteExpense, addIncome, updateIncome, deleteIncome, partners } = useAppStore();
  const [activeSubTab, setActiveSubTab] = useState('finance');

  // Add modal states for income & expenses
  const [showExpenseModal, setShowExpenseModal] = useState(false);
  const [showIncomeModal, setShowIncomeModal] = useState(false);
  const [editingExpense, setEditingExpense] = useState(null);
  const [editingIncome, setEditingIncome] = useState(null);

  // Add form states for income & expenses
  ...
  // Add filters for income & expenses
  ...
  // Handler functions for income & expenses
      ...
      {activeSubTab === 'income-expenses' && (
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900">Income & Expenses Management</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="card p-6">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900">€{filteredIncome.reduce((sum, inc) => sum + inc.amount, 0).toFixed(2)}</p>
            ...
            <div className="card p-6">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900">€{filteredExpenses.reduce((sum, exp) => sum + exp.amount, 0).toFixed(2)}</p>
            ...
            <div className="card p-6">
              <div className="flex items-center">
                <div className={`${(filteredIncome.reduce((sum, inc) => sum + inc.amount, 0) - filteredExpenses.reduce((sum, exp) => sum + exp.amount, 0)) >= 0 ? 'bg-emerald-500' : 'bg-orange-500'} rounded-lg p-3 text-white flex items-center justify-center mr-4`}>
                  <span className="text-lg font-bold">€</span>
                </div>
                <div>
                  <p className={`text-2xl font-bold ${(filteredIncome.reduce((sum, inc) => sum + inc.amount, 0) - filteredExpenses.reduce((sum, exp) => sum + exp.amount, 0)) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    €{(filteredIncome.reduce((sum, inc) => sum + inc.amount, 0) - filteredExpenses.reduce((sum, exp) => sum + exp.amount, 0)).toFixed(2)}
                  </p>
            ...
            <div className="card p-6">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-blue-600">{filteredExpenses.length + filteredIncome.length}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Recent Expenses ({filteredExpenses.length})</h3>
              </div>
              <div className="overflow-x-auto">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Recent Income ({filteredIncome.length})</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="table">
                  <thead>
                  ...
                  <tbody>
                    {filteredIncome.slice(0, 10).map((inc) => (
                      <tr key={inc.id}>
                        <td>{inc.date}</td>
